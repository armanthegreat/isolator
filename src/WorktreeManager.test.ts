...
describe("generateTempBranchName", () => {
  it("returns a string in isolator/<YYYYMMDD-HHMMSS> format", () => {
    const name = generateTempBranchName();
    expect(name).toMatch(/^isolator\/\d{8}-\d{6}$/);
  });
  ...
  it("includes sanitized name when provided", () => {
    ...
    expect(name).toMatch(/^isolator\/my-run\/\d{8}-\d{6}$/);
  });
...
describe("WorktreeManager.create", () => {
  it("creates a worktree at .isolator/worktrees/<name>/", async () => {
    const repoDir = await setupRepo();
    const { path } = await run(create(repoDir));
    expect(path).toContain(join(repoDir, ".isolator", "worktrees"));
  ...
  it("the worktree directory is on the correct branch", async () => {
    ...
    expect(stdout.trim()).toMatch(/^isolator\//);
  });
  ...
  it("creates a new branch from HEAD when specified branch does not exist", async () => {
    const repoDir = await setupRepo();
    const { path, branch } = await run(
      create(repoDir, { branch: "isolator/issue-42-new-feature" }),
    );

    expect(branch).toBe("isolator/issue-42-new-feature");
    expect(await getBranch(path)).toBe("isolator/issue-42-new-feature");
  ...
  it("does not write upstream tracking config even when autoSetupMerge is enabled", async () => {
    ...
    const { path, branch } = await run(
      create(repoDir, { branch: "isolator/no-tracking-test" }),
    );
    ...
    const { stdout: trackingConfig } = await execAsync(
      `git config --get-regexp "branch\\.isolator/no-tracking-test\\." || true`,
      { cwd: repoDir },
    );
...
describe("WorktreeManager.pruneStale", () => {
  ...
  it("removes orphaned directories under .isolator/worktrees/", async () => {
    const repoDir = await setupRepo();
    const worktreesDir = join(repoDir, ".isolator", "worktrees");
  ...
  it("does not remove active worktrees when .isolator is a symlink", async () => {
    // Regression test for #470: git canonicalizes worktree paths, so when
    // .isolator is a symlink the un-canonicalized entryPath never matched
    ...
    await symlink(externalDir, join(repoDir, ".isolator"));
