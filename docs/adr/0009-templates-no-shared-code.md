...
This ADR exists because the question recurs in PRs — e.g. a contributor proposing `import { extractPlanIssues } from "isolator/utils"` to dedupe a `<plan>…</plan>` regex parse between `parallel-planner` and `parallel-planner-with-review`. The answer is: copy the helper into each template's `main.mts`.

## Considered Options

1. **Promote shared helpers to public exports** (`isolator/utils`) — rejected. Grows the public API to support template internals, locks in helper signatures the wider ecosystem doesn't need, and couples templates to each other through the package.
...
4. **Each template directory is self-contained, imports only `isolator`** (chosen). One-line rule, easy to apply on review, keeps `init` a pure directory copy, and lets each template evolve independently.
...
- `isolator init <template>` remains a verbatim copy of `src/templates/<name>/` — no dependency resolution, no inlining step.
