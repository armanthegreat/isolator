# Isolator low-level library API

Isolator's brain layer (`runStep()`, pipelines, the `isolator` CLI — see the
[README](../README.md)) is the documented, supported surface for everyday use.

This document covers the **lower-level execution core** — `run()`,
`createSandbox()`, `createWorktree()`, and the sandbox-provider contract. It is a
bare embed path: no brain, no vault, no telemetry. Reach for it only when you are
embedding the engine directly in your own tooling. The brain layer is built on
top of exactly these primitives.

Environment variables are resolved from the central `~/.isolator/.env`
(see [Authentication](../README.md#authentication)); there is no per-repo `.env`.

## Sandbox Providers

Isolator uses a `SandboxProvider` to create isolated environments. The `sandbox`
option on `run()` and `createSandbox()` accepts any provider. A no-sandbox option
is also available for `interactive()` and `wt.interactive()`. Built-in providers:
...
const result = await run({
  ...
  // .isolator/ artifacts (worktrees, logs, patches) and git operations.
  ...
  promptFile: "./prompt.md", // path to a prompt file
...
Isolator uses a **branch strategy** configured on `run()` to control how the agent's changes relate to branches. There are three strategies:
...
Use `Output.object()` to extract a typed, schema-validated JSON payload from the agent's stdout. The agent emits its answer inside an XML tag you specify, and Isolator parses, validates, and returns it on `result.output`. See [ADR 0010](adr/0010-structured-output.md) for design rationale.
...
Environment variables are also resolved automatically from `~/.isolator/.env` and `process.env` — no need to pass them to the API.
...
### Passing a custom provider to `run()`
